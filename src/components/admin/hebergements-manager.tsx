...
const formSchema = z.object({
  ...
  localisation: z.string().min(2, 'Veuillez entrer une localisation.'),
  nombreEtoiles: z.coerce.number().min(0).max(5),
  ...
  async function onSubmit(values: FormValues) {
    ...
    try {
      ...
      const dataToSave = {
        ...
        nombreEtoiles: values.nombreEtoiles,
            ...
            <Form {...form}>
              <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
                <FormField control={form.control} name="titre" render={({ field }) => (
                  <FormItem><FormLabel>Titre</FormLabel><FormControl><Input placeholder="Hôtel Terrou-Bi" {...field} /></FormControl><FormMessage /></FormItem>
                )} />
                ...
                <div className="grid grid-cols-2 gap-4">
                  <FormField control={form.control} name="prix" render={({ field }) => (
                    <FormItem><FormLabel>Prix</FormLabel><FormControl><Input placeholder="À partir de 75 000 XOF" {...field} /></FormControl><FormMessage /></FormItem>
                  )} />
                  <FormField control={form.control} name="tag" render={({ field }) => (
                    <FormItem><FormLabel>Catégorie</FormLabel><FormControl><Input placeholder="Luxe" {...field} /></FormControl><FormMessage /></FormItem>
                  )} />
                </div>
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <FormField control={form.control} name="localisation" render={({ field }) => (
                    <FormItem><FormLabel>Localisation</FormLabel><FormControl><Input placeholder="Dakar" {...field} /></FormControl><FormMessage /></FormItem>
                  )} />
                </div>
                <div className="grid grid-cols-2 gap-4">
                  <FormField control={form.control} name="nombreEtoiles" render={({ field }) => (
                    <FormItem><FormLabel>Nombre d'étoiles (0-5)</FormLabel><FormControl><Input type="number" min={0} max={5} {...field} /></FormControl><FormMessage /></FormItem>
                  )} />
                  <FormField control={form.control} name="capacite" render={({ field }) => (
                    <FormItem><FormLabel>Capacité</FormLabel><FormControl><Input placeholder="2 adultes" {...field} /></FormControl><FormMessage /></FormItem>
                  )} />
                </div>
                <FormField control={form.control} name="equipements" render={({ field }) => (
                  <FormItem><FormLabel>Équipements (séparés par virgules)</FormLabel><FormControl><Input placeholder="WiFi, Piscine, Petit-déjeuner" {...field} /></FormControl><FormMessage /></FormItem>
                )} />
                <div className="grid grid-cols-2 gap-4">
        ...
        <Table>
          ...
          <TableBody>
            {isLoading ? (
              ...
              <TableRow><TableCell colSpan={6} className="text-center h-24">Aucun hébergement trouvé.</TableCell></TableRow>
            )}
          </TableBody>
        </Table>
      </div>
