...
export default function ExcursionsManager() {
  ...
  const form = useForm<FormValues>({
    ...
    defaultValues: {
      titre: '', description: '', descriptionComplete: '', prix: '', tag: '', lieu: '',
      duree: '', pointDepart: '', difficulte: 'facile', inclus: '',
  ...
  async function onSubmit(values: FormValues) {
    ...
    try {
      ...
      if (editingItem) {
        ...
        toast({ title: 'Succès !', description: "L'excursion a été modifiée." });
      } else {
    ...
    <div>
      <div className="flex justify-between items-center mb-6">
        <div>
          ...
          <p className="text-muted-foreground">Ajoutez et modifiez les excursions.</p>
        </div>
        <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
          ...
          <DialogContent className="sm:max-w-[560px] max-h-[90vh] overflow-y-auto">
            <DialogHeader>
              <DialogTitle>{editingItem ? 'Modifier' : 'Nouvelle'} Excursion</DialogTitle>
              <DialogDescription>Remplissez les informations ci-dessous.</DialogDescription>
            </DialogHeader>
            <Form {...form}>
              <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
                <FormField control={form.control} name="titre" render={({ field }) => (
                  <FormItem><FormLabel>Titre</FormLabel><FormControl><Input placeholder="Île de Gorée — Mémoire et patrimoine" {...field} /></FormControl><FormMessage /></FormItem>
                )} />
                <FormField control={form.control} name="description" render={({ field }) => (
                  <FormItem><FormLabel>Description courte</FormLabel><FormControl><Textarea placeholder="À 20 minutes de bateau de Dakar..." {...field} /></FormControl><FormMessage /></FormItem>
                )} />
                ...
                <div className="grid grid-cols-2 gap-4">
                  <FormField control={form.control} name="prix" render={({ field }) => (
                    <FormItem><FormLabel>Prix</FormLabel><FormControl><Input placeholder="Dès 15 000 FCFA/personne" {...field} /></FormControl><FormMessage /></FormItem>
                  )} />
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <FormField control={form.control} name="duree" render={({ field }) => (
                    <FormItem><FormLabel>Durée</FormLabel><FormControl><Input placeholder="Demi-journée (4h)" {...field} /></FormControl><FormMessage /></FormItem>
                  )} />
                </div>
                <div className="grid grid-cols-2 gap-4">
                ...
                <FormField control={form.control} name="inclus" render={({ field }) => (
                  <FormItem><FormLabel>Inclus (séparés par virgules)</FormLabel><FormControl><Input placeholder="Transport, Guide, Repas, Entrée site" {...field} /></FormControl><FormMessage /></FormItem>
                )} />
                <div className="grid grid-cols-2 gap-4">
