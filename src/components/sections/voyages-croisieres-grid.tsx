...
import { ArrowRight, Eye, Info, Loader2, Ship, Plane } from 'lucide-react';
...
export default function VoyagesCroisieresGrid() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const snapshot = await getDocs(query(collection(getDbInstance(), 'voyagesCroisieres'), orderBy('ordre', 'asc')));
        const data: Offre[] = [];
        ...
        if (mounted) {
          ...
          if (categoryParam === 'voyage' || categoryParam === 'croisiere') setActiveCategory(categoryParam);
          else {
            const firstWithItems = CATEGORIES.find((category) => data.some((item) => item.type === category.key));
            if (firstWithItems) setActiveCategory(firstWithItems.key);
          }
        }
      } catch (error) {
        console.error('Erreur fetch voyages et croisières:', error);
      } finally {
